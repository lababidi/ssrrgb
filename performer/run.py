...
def model0(img, name):
    ...
    for x in np.arange(0, img.shape[0], size):
        for y in np.arange(0, img.shape[1], size):
            ...
            new_chunk0 = model.predict(np.stack([np.expand_dims(chunk[:,:,0:1], 0),np.expand_dims(chunk[:,:, 1:2], 0)]))[0]
            new_img[2 * x:2 * x + 2 * size, 2 * y:2 * y + 2 * size, 1] = new_chunk0[:, :, 0]
            new_chunk1 = model.predict(np.expand_dims(chunk[:,:, 1:2], 0))[0]
            new_img[2 * x:2 * x + 2 * size, 2 * y:2 * y + 2 * size, 1] = new_chunk1[:, :, 0]
            new_chunk2 = model.predict(np.expand_dims(chunk[:,:, 2:], 0))[0]
            new_img[2 * x:2 * x + 2 * size, 2 * y:2 * y + 2 * size, 2] = new_chunk2[:, :, 0]
    print(new_img)
    #
